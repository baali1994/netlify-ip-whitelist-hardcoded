...
IPv4-only whitelist update
--------------------------
- New whitelist entries accept IPv4 only.
- The Netlify check/whoami logic searches Netlify/client headers for an IPv4 address and ignores plain IPv6 addresses.
- The admin Detect IPv4 button falls back to the IPv4-only ipify endpoint if Netlify only exposes IPv6 to the browser request.
- Existing legacy IPv6 rows can still be disabled or deleted from the admin table.
